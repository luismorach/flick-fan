...
import { episode, listSeries, season, Serie } from '../../interfaces/interfaces';
...
import { CarouselSeriesComponent } from '../carousel-series/carousel-series.component';
register()
@Component({
...
export default class PlaySerieComponent implements AfterViewInit {
  ...
  getUrlSerie() {
    ...
    this.subscription.push(params.pipe(
      map(([params, queryParams]) => {
        ...
        return this.API.getDetailsSerie(this.id_serie)
      }), concatAll())
  ...
  getSimilarSeries() {
    this.subscription.push(this.rutaActiva.params.pipe(
      map((params: Params) => this.API.getSimilarSeries(1, params['id_serie'])), concatAll())
  ...
  getRecomendedSeries() {
    this.subscription.push(this.rutaActiva.params.pipe(
      map((params: Params) => this.API.getRecomendedSeries(1, params['id_serie'])), concatAll())
      .subscribe((series: any) => {
        if (series.results.length > 0) {
